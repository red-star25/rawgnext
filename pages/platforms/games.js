...
const PlatformGames = ({ gamesData, page, name }) => {
    ...
    <div>
      <div className="w-[100%] bg-black px-5">
        ...
        <div className=" mt-10 w-[100%] h-[100%] flex ">
          ...
          <div>
            <div className="hidden md:block w-[100%]  ml-4 mb-4 lg:w-[78%]">
              ...
              {page <= 1 ? (
                ...
                <div className="mr-2 mt-5 mb-3 flex text-white items-center justify-center w-[fit-content] p-1 px-3 self-center rounded-sm border-white border hover:bg-white hover:text-black transition duration-300 cursor-pointer">
                  <button
                    ...
                    onClick={() =>
                      router.push(
                        `/platforms/games?name=${name}&page=${page - 1}`
                      )
                    }
                  >
                    Previous
...
export async function getServerSideProps({ query: { name, page } }) {
  const platformTag = {
    pc: 4,
    playstation: 1,
    xbox: 3,
  ...
  const res = await fetch(
    `https://api.rawg.io/api/games?key=${process.env.API_KEY}&platforms=${platformTag[name]}&page=${page}`
  );
  const data = await res.json();
  console.log(data);
  return {
    props: { gamesData: data, page: +page, name: name },
  };
}
